...
반복 충돌이 발생하면 먼저 `git status`로 충돌 파일을 확인하고, `git diff`로 충돌 hunk가 어느 문단이나 함수에 몰려 있는지 본다. 같은 문단을 여러 PR이 동시에 수정했다면 작업 범위 중복이 원인이다. 파일 이동과 내용 수정이 동시에 있었다면 경로 변경 순서가 원인이다.

원인별 대응은 다음과 같다.

| 원인 | 확인 방법 | 예방 |
| --- | --- | --- |
| README 공통 문단 동시 수정 | 같은 hunk에 여러 PR 변경이 모임 | 한 PR을 기준 문장으로 먼저 병합하고 나머지는 최신 `main` 기준으로 수정 |
| 제출 인덱스 표 동시 수정 | 같은 표 행이나 링크 칸에 충돌 발생 | 팀원별 행 담당자를 정하고 리뷰 전 최신 상태 확인 |
| 파일 이동과 내용 수정 병행 | modify/delete 또는 rename 관련 충돌 발생 | 경로 변경 PR과 내용 수정 PR을 분리 |
| 요구 파일명 변경 | 과제 요구 경로와 실제 경로가 달라짐 | 파일명 변경 전 이슈에서 요구사항과 링크 영향 확인 |

충돌 해결 후에는 마커 제거만 확인하지 않는다. 선택한 해결 전략, 실제 명령, 테스트 결과, 다음 예방 조치를 이 파일에 남긴다.
