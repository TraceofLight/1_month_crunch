...
`3`번 패턴 생성기 모드에서는 홀수 크기 `N`을 입력한 뒤 Cross 또는 X 패턴을 바로 생성할 수 있고, 생성된 패턴으로 2D/1D MAC 성능 비교를 즉시 확인할 수 있습니다.
...
- 패턴 생성기 메뉴(`3`)를 추가해 사용자가 크기 `N`과 Cross/X를 선택하면 생성된 패턴을 콘솔에서 바로 확인할 수 있게 했습니다.

- 생성된 패턴은 즉시 2D/1D MAC 성능 비교에 재활용할 수 있고, 원하면 `data.json`의 `patterns`에 추가 저장할 수 있습니다.

- 이때 해당 크기의 관련 필터가 `data.json`에 없으면 mode 2 분석에서 프로그램이 중단되지 않고, `관련 필터가 없어 판정할 수 없음` 사유와 함께 FAIL로 집계됩니다.

- `data.json`이 없을 때 기본 데이터셋을 자동 생성해 바로 실행 가능한 상태를 보장합니다.

- 실패 케이스도 숨기지 않고 `reason` 필드로 기록해 디버깅과 결과 해석이 쉽도록 했습니다.

  ![creation_not_save](./media/creation_not_save.png)

  ![creation_save](./media/creation_save.png)
...
- 패턴 생성기에서 새로 저장한 `size_7_1` → `N = 7` 추출 → `filters["size_7"]` 탐색
...
패턴 생성기로 추가한 케이스도 같은 `size_{N}_{idx}` 규칙을 따르므로 mode 2에서 동일한 분석 흐름을 그대로 재사용할 수 있습니다. 다만 해당 크기의 관련 필터가 아직 `filters`에 없으면, 그 케이스는 프로그램 중단 없이 `관련 필터가 없어 판정할 수 없음` 사유와 함께 FAIL로 기록됩니다.
...
- 콘솔 입력 검증과 재시도 흐름
- 패턴 생성기 메뉴 동작
- 생성 패턴의 `data.json` 추가 저장
- 관련 필터가 없는 생성 패턴의 FAIL 처리
- `Ctrl+C`, `Ctrl+D` 종료 처리

![test_result](./media/test_result.png)

### 10-4. 패턴 생성기 검증
다음 명령으로 패턴 생성기 모드를 검증했습니다.
...
검증 결과:
- 메뉴 `3. 패턴 생성기 (보너스)`가 정상 출력됨을 확인
- `5x5` Cross 패턴이 콘솔에 정상 출력됨을 확인
- 생성 직후 2D 평균 시간과 1D 평균 시간이 함께 출력됨을 확인
- `n` 선택 시 `data.json` 저장 없이 종료 흐름이 이어짐을 확인
