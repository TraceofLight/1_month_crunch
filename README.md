...
## 4. 파일 구조
```text
d:/Projects/Github/1_month_crunch/.worktrees/e1-3-mini-npu
├── README.md
├── data.json
├── docs/
├── main.py
...
즉, 이 두 케이스는 우연한 오답 데이터가 아니라 동점 판정 정책과 입력 검증 정책을 문서화된 형태로 드러내기 위해 `data.json`에 intentionally 포함된 검증용 사례입니다.
...
- 오직 `benchmark_mac()` 내부에서 반복 호출되는 `mac_2d()` 또는 `mac_1d()` 연산 시간만 평균값으로 측정합니다.
...
- 제출 문서는 구현과 정확히 일치해야 하므로, README에는 실제 실행으로 다시 확인한 최신 결과만 반영했습니다.
