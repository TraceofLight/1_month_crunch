...
- 베이스: `ubuntu:22.04` (요구사항 §6의 Ubuntu 22.04 LTS와 동일)
...
### 1.1 SSH 포트 변경(20022) + Root 원격 로그인 차단
...
접근 통제(2.3)에서 결정적으로 작동한다.

### 2.2 디렉터리 구조 + 소유/권한 + ACL
...
# bin 내부 (monitor.sh 권한 정책 — §3.4 참고)
...
> 요구사항 §8의 monitor.sh "예시 출력"에는 통계(STATISTICS REPORT)가 함께 보이지만, §4의 기능
> 요구는 monitor.sh 에 수집·로깅까지만 요구한다. 통계 분석은 책임을 분리해 보너스 1의 `report.sh`
> 가 담당하도록 했다(단일 책임 — monitor.sh 는 "수집기", report.sh 는 "분석기").

### 3.4 프로세스 식별 · 포트 확인 — 명령 선택과 이유
...
`agent-admin` 은 `agent-core` 에 속하므로(2.1의 `id` 결과: `1001(agent-core)`) 소유자가 아니어도
...
`/var/log/agent-app` 는 `agent-core` 그룹 R/W(2.2)이므로 agent-admin 이 로그를 남길 수 있다 —
...
### 3.7 경고(WARNING)와 종료(exit) 항목 분리 — 운영 이유
...
이후 cron 으로 라인이 누적되는 모습은 §5에서 확인한다.

### 4.2 리다이렉션 기호 차이(`>` vs `>>`)와 누적 필요성
...
### 4.3 로그 용량 관리 (size 기반 회전)

요구사항의 "최대 10MB / 10개 파일"을 스크립트 로직으로 구현했다(logrotate 데몬 의존 없이 cron
한 번에 끝나도록).
...
### 7.2 디렉터리 권한 제한과 최소 권한 원칙

최소 권한(least privilege)은 "각 주체에게 그 역할에 꼭 필요한 권한만" 부여하는 원칙이다. 본 구성은
이를 그룹 분리 + ACL 로 구현했다.
...
- **공유 vs 보안 분리.** `upload_files` 는 협업 그룹 `agent-common`(admin/dev/test) 전원이 R/W
  ...
  못한다(§2.3에서 `Permission denied` 로 실증). 키가 유출되거나 test 계정이 탈취돼도 보안 자원은
...
### 7.4 프로세스·포트 상태가 불일치할 때 — 원인과 확인 순서
