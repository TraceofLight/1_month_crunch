...
BFS는 간선 수가 가장 적은 경로를 먼저 발견하는 탐색이다. 인접 커밋을 해시 사전순으로 탐색하고, 각 커밋의 최초 방문 부모를 기록한다. 따라서 최단 경로가 여러 개여도 사전순이 가장 작은 경로가 선택된다. 목표에 도달하면 부모 포인터를 역추적해 경로를 복원하므로 탐색 비용은 O(V + E)이다.
...
`ANCESTORS <commit_hash>`는 지정 커밋의 부모 방향으로 도달 가능한 모든 커밋을 출력한다. 구현은 큐 기반 BFS로 부모를 따라가며 방문 집합으로 중복을 제거한다. 특정 커밋의 조상 수를 A, 조상 사이의 부모 간선을 E_A라고 하면 시간복잡도는 O(A + E_A)이다.
