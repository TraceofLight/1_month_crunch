...
2026-06-08 검증 시점에 배포 URL은 HTTP 200을 반환했고 현재 산출물의 식별자인 `TraceofLight Portfolio` 또는 `Frontend Portfolio`를 포함했다. `evidence/pages-check.txt`에는 Pages API가 HTTP 404를 반환했고 Pages URL은 HTTP 200과 현재 페이지 식별자 포함=True를 반환한 결과가 저장되어 있다. 현재 저장소에는 `.github/workflows/pages.yml`이 포함되어 있으며, `b4-1` 브랜치가 푸시되면 GitHub Pages로 정적 파일을 배포하도록 구성했다.
...
GitHub Pages 배포는 `.github/workflows/pages.yml`이 담당한다. workflow는 `contents: read`, `pages: write`, `id-token: write` 권한만 선언하고, 정적 파일 전체를 Pages artifact로 업로드한 뒤 `actions/deploy-pages@v4`로 배포한다.
...
페이지는 Hero, About, Skills, Projects, Contact, Footer로 구성되어 있다. Hero에는 인사말과 프로젝트 및 문의 섹션으로 이동하는 CTA 버튼이 있고, About에는 자기소개 성격의 작업 방식 설명과 프로필 시각 자료가 있다. Skills에는 HTML5 Semantic Tags, CSS Variables, Flexbox, CSS Grid, JavaScript ES6+, Fetch API, LocalStorage, Intersection Observer 기술 목록이 표시된다. Projects는 GitHub API 응답을 카드로 렌더링하고, Contact는 이름, 이메일, 메시지를 받는 문의 폼을 제공한다. Footer에는 2026 TraceofLight 저작권 문구와 GitHub, LinkedIn 소셜 링크가 있다.
...
`index.html`은 외부 스타일시트 `css/style.css`를 연결하고, JavaScript 파일 `js/main.js`를 `defer` 속성으로 불러온다. 이 방식은 HTML 파싱을 막지 않으면서 DOM이 준비된 뒤 이벤트 연결과 초기 렌더링이 실행되게 한다.
...
모바일 퍼스트를 선택한 이유는 제약이 큰 화면을 기본값으로 삼기 위해서다. 작은 화면에서는 내비게이션이 접혀야 하고, 텍스트와 카드가 한 열로 쌓여야 하며, 터치 영역도 충분히 커야 한다. 이 조건을 기본 스타일로 먼저 만족시키면 넓은 화면에서는 768px 이상에서 2열 레이아웃을 추가하고 1024px 이상에서 데스크톱 내비게이션을 펼치는 식으로 확장만 하면 된다. 데스크톱 스타일을 먼저 작성한 뒤 모바일에서 덜어내는 방식보다 중복 override가 적고, 실제 사용자가 모바일에서 처음 접속해도 핵심 콘텐츠가 먼저 안정적으로 보인다.

모바일에서는 네비게이션 메뉴가 숨겨지고 햄버거 버튼이 나타난다. 1024px 이상에서는 데스크톱 메뉴가 항상 보이고 햄버거 버튼은 숨겨진다. 버튼과 카드에는 `transition`, `hover`, `box-shadow`를 적용해 클릭 가능한 요소와 정보 카드가 시각적으로 구분되도록 했다.

## 이벤트와 DOM 조작

DOM 선택은 `querySelector`와 `querySelectorAll`을 사용한다. 선택한 요소에는 HTML의 `onclick` 속성을 쓰지 않고 `addEventListener`로 `click`, `submit`, `scroll`, `input` 이벤트를 연결했다. `onclick`은 동작을 HTML 속성에 직접 섞는다. 그러면 마크업을 읽는 사람이 화면 구조와 실행 로직을 동시에 해석해야 하고, 같은 요소에 같은 이벤트의 여러 처리를 붙이기도 어렵다. `addEventListener`는 JavaScript 파일 안에서 이벤트 연결을 관리하므로 HTML은 구조에 집중하고 JavaScript는 동작에 집중한다. 또한 한 요소에 여러 리스너를 붙일 수 있고, `querySelectorAll`로 고른 여러 앵커에 `forEach`로 같은 스크롤 동작을 반복 연결할 수 있어 메뉴 링크와 CTA 링크를 일관되게 처리하기 쉽다.

이벤트 핸들러는 상태 객체를 바꾸고, 렌더링 함수가 `textContent`, `innerHTML`, `classList.add`, `classList.remove`, `classList.toggle`로 화면을 갱신한다. 모바일 햄버거 메뉴는 버튼 클릭 때 `classList.toggle('active')`로 열고 닫으며, 메뉴 상태에 따라 `aria-expanded`와 `aria-label`도 갱신한다. 네비게이션 링크와 Hero CTA를 클릭하면 기본 앵커 이동을 `event.preventDefault()`로 막고 `scrollIntoView`로 해당 섹션까지 부드럽게 이동한다.

스크롤 위치가 300px 이상이면 스크롤 탑 버튼이 보이고, 버튼을 클릭하면 `window.scrollTo`로 페이지 맨 위까지 부드럽게 이동한다. 스크롤 위치가 60px 이상이면 헤더에 `scrolled` 클래스가 붙어 배경색, 테두리, 그림자가 바뀐다. 스크롤 애니메이션은 `IntersectionObserver`로 구현했고, `OBSERVER_THRESHOLD`는 0.25다. 각 섹션이 25% 이상 보이면 `visible` 클래스가 붙어 opacity와 transform 전환이 실행된다.

화살표 함수는 짧은 이벤트 핸들러와 렌더링 헬퍼를 간결하게 표현하기 위해 사용했다. 구조분해 할당은 GitHub 저장소 객체에서 `name`, `description`, `html_url`, `language`, `stargazers_count`, `updated_at` 같은 필드를 꺼낼 때 사용했다. `map`은 저장소 배열을 프로젝트 카드 HTML로 변환하고, `filter`는 fork 저장소 제외와 언어 필터에 사용하며, `forEach`는 여러 링크와 폼 입력에 이벤트를 연결할 때 사용한다. JavaScript는 `var`를 쓰지 않고 `const`와 `let`만 사용한다.
...
Hero 제목에는 “TraceofLight입니다.”가 한 글자씩 나타나는 타이핑 효과가 적용되어 있다. 이 효과는 `setTimeout`으로 현재 인덱스까지의 문자열을 `textContent`에 반복 반영한다.

## 프로젝트 데이터와 API

GitHub API 엔드포인트는 `https://api.github.com/users/TraceofLight/repos`다. `fetch`와 `async/await`로 저장소 목록을 가져오고, 로딩, 성공, 에러, 빈 상태를 Projects 섹션에 렌더링한다. 로딩 중에는 스피너와 “로딩 중...” 문구가 보인다. 성공하면 카드 리스트가 보이고, 각 카드는 저장소 이름, 설명, 언어, 별 개수, 업데이트 날짜, 저장소 링크를 표시한다. 에러가 발생하면 “프로젝트를 불러올 수 없습니다.” 메시지와 재시도 버튼이 보인다. 403 응답에서는 “GitHub API 호출 한도에 도달했습니다.” 메시지를 표시할 수 있다. 필터 결과가 비었거나 표시할 저장소가 없으면 “표시할 프로젝트가 없습니다.” 문구를 보여준다.
...
프로젝트 필터링은 GitHub 응답에서 언어 값을 모아 필터 버튼을 만들고, 사용자가 언어를 선택하면 `state.activeLanguage`를 바꾼 뒤 `filter`로 프로젝트 목록을 다시 계산하는 방식이다. `all` 필터는 전체 저장소를 보여주고, 언어별 필터는 해당 언어와 일치하는 저장소만 보여준다.
...
인증 없이 GitHub API를 호출하므로 시간당 60회 제한을 받는다. 짧은 시간에 반복 새로고침하면 403 응답이 발생할 수 있고, 이 경우 Projects 섹션은 에러 상태 UI와 재시도 버튼을 표시한다.

## 문의 폼

Contact 섹션의 문의 폼은 이름, 이메일, 메시지를 입력받는다. 세 필드는 모두 필수값이고, 이메일은 `name@example.com` 형식이어야 한다. 오류 메시지는 각 입력 필드 근처에 표시된다.

폼 제출은 `event.preventDefault()`로 브라우저 기본 제출을 막는다. 검증 오류가 있으면 성공 메시지를 비우고 필드별 오류만 보여준다. 오류가 없으면 폼을 초기화하고 “문의가 접수되었습니다.” 성공 메시지를 표시한다. 현재 폼은 실제 전송을 하지 않으므로 개인정보가 외부 서비스로 나가지 않는다.

## 보안과 운영

외부 입력은 GitHub API 응답과 문의 폼 입력이다. GitHub 저장소 이름, 설명, URL, 언어 값은 외부 데이터로 보고 HTML 삽입 전에 이스케이프한다. URL과 텍스트가 카드 템플릿에 들어가기 전에 `escapeHtml`을 거치므로 저장소 설명에 HTML 문자가 포함되어도 페이지 구조를 깨지 않는다.
...
운영 중 프로젝트 카드가 보이지 않으면 브라우저 개발자 도구의 Network 탭에서 `https://api.github.com/users/TraceofLight/repos` 응답을 확인한다. 403이면 인증 없는 GitHub API 호출 한도에 걸린 상태이므로 시간을 두고 다시 시도한다. 네트워크 오류면 Projects 섹션의 재시도 버튼을 누른다.
...
| `evidence/pages-check.txt` | GitHub Pages API와 공개 URL 확인 결과다. Pages API는 HTTP 404, Pages URL은 HTTP 200, 현재 페이지 식별자 포함=True를 기록했다. |
...
`evidence/run-log.txt`의 전체 검증 로그는 다음과 같다.

```text
검증 시각: 2026-06-08T01:29:55.261229+00:00
정적 요구사항 검사: 통과
GitHub API 응답 성공: HTTP 200, 저장소 25개
GitHub Pages 확인 결과 저장: evidence/pages-check.txt
desktop.png 캡처 완료
mobile.png 캡처 완료
dark-mode.png 캡처 완료
모든 자동 검증 항목 통과
...
`evidence/pages-check.txt`의 전체 확인 결과는 다음과 같다.

```text
GitHub Pages 외부 배포 확인
Pages API: HTTP 404
Pages URL: HTTP 200, 현재 페이지 식별자 포함=True
```
